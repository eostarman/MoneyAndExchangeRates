...
public struct MoneyWithoutCurrency: Codable, Hashable {
    ...
    public var formattedAs2Or4Decimals: String {
        ...
        var numberOfDecimals = self.numberOfDecimals
        
        if numberOfDecimals == 4 && scaledAmount % 100 == 0 {
            numberOfDecimals = 2
        }
...
extension MoneyWithoutCurrency {
    ...
    public func divided(by divisor: Int, numberOfDecimals: Int) -> MoneyWithoutCurrency {
        MoneyWithoutCurrency(amount: amount * (1.0 / Double(divisor)), numberOfDecimals: numberOfDecimals)
    }
    
    /// the result is 4 decimals
    public static func * (money: MoneyWithoutCurrency, multiplier: Double) -> MoneyWithoutCurrency {
        MoneyWithoutCurrency(amount: money.amount * multiplier, numberOfDecimals: 4)
    }
    public static func * (multiplier: Double, money: MoneyWithoutCurrency) -> MoneyWithoutCurrency {
        MoneyWithoutCurrency(amount: money.amount * multiplier, numberOfDecimals: 4)
    }
    
    public static func * (qty: Int, money: MoneyWithoutCurrency) -> MoneyWithoutCurrency {
        MoneyWithoutCurrency(scaledAmount: money.scaledAmount * Int64(qty), numberOfDecimals: money.numberOfDecimals)
    }
    
    public static func * (money: MoneyWithoutCurrency, qty: Int) -> MoneyWithoutCurrency {
        MoneyWithoutCurrency(scaledAmount: money.scaledAmount * Int64(qty), numberOfDecimals: money.numberOfDecimals)
    }
}

extension Optional where Wrapped == MoneyWithoutCurrency {
    public static func * (qty: Int, money: MoneyWithoutCurrency?) -> MoneyWithoutCurrency? {
        guard let money = money else {
            return nil
        }
        return MoneyWithoutCurrency(scaledAmount: money.scaledAmount * Int64(qty), numberOfDecimals: money.numberOfDecimals)
    }
...
extension String.StringInterpolation {
    ...
    public mutating func appendInterpolation(_ value: MoneyWithoutCurrency) {
        appendInterpolation(value.formattedAs2Or4Decimals)
    }
}
